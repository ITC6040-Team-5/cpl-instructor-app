...
def compute_completion(case_data, message_count=0, evidence_count=0):
    """
    Compute case completion percentage (0–100).

    Scoring model (v2 — honest, not inflated):
      - 0% base: fresh case, nothing meaningful yet
      - Name confirmed in conversation (+10%)
      - Student ID confirmed in conversation (+15%)
      - Target course identified (+20%)
      - Prior learning narrative — substantive messages (+20%)
        Only counts when identity is present AND message_count >= 6
      - Evidence uploaded (+15%)
      - Summary generated by extraction (+20%)

    localStorage identity does NOT pre-inflate score.
    A "What is this?" first message stays near 0%.
    """
    pct = 0
    ...
    has_sid = bool(sid and len(str(sid).strip()) >= 6)
    has_course = bool(course and course not in ("Not yet determined", "—", None, ""))
    has_identity = has_name or has_sid

    # 1. Name provided and validated (+10%)
    if has_name:
        pct += 10

    # 2. Student ID provided and validated (+15%)
    if has_sid:
        pct += 15

    # 3. Target course identified (+20%)
    if has_course:
        pct += 20

    # 4. Prior learning narrative — only when identity confirmed AND 6+ messages
    # This prevents early inflation from generic greetings
    if has_identity and has_course:
        if message_count >= 8:
            pct += 20
            ...
            pct += 8
    elif has_identity and not has_course:
        # Partial credit for substantive exploration without course yet
        if message_count >= 6:
            pct += 5
...
def update_case(case_id, updates):
    ...
    try:
        ...
        allowed = {
            ...
            "conversation_summary", "claimed_competencies",
        }
...
def transition_status(case_id, new_status, notes=None):
    ...
    valid_transitions = {
        ...
        "Under Review": ["Approved", "Denied", "Revision Requested", "Escalated"],
        "Revision Requested": ["In Progress", "Submitted"],
        "Escalated": ["Under Review", "Approved", "Denied"],
    }
...
def _row_to_case(row):
    ...
    return {
        ...
        "claimed_competencies": getattr(row, 'claimed_competencies', None),
